...
import {AlertCircle, ArrowLeft, Download, Loader2, Palette ,Check,Save, Trash2} from "lucide-react";
...
const EditResume = () => {
  ...
  if (!resumeId) {
    toast.error("Resume ID is missing. Cannot upload images.");
    return;
  }
  ...
  try {
    ...
    const uploadResponse = await axiosInstance.post(API_PATHS.RESUMES.UPLOAD_IMAGES(resumeId), formData, { headers: { "Content-Type": "multipart/form-data" }});
    ...
    try {
      ...
      console.error("Error updating resume details:", updateError);
      toast.error("Failed to update resume details after upload");
    }
  } catch (error) {
...
const fetchResumeDetailsById = async () => {
  if (!resumeId) {
    toast.error("Resume ID is missing. Cannot fetch resume details.");
    return;
  }
...
const updateResumeDetails = async (thumbnailLink) => {
  if (!resumeId) {
    toast.error("Resume ID is missing. Cannot update resume.");
    return;
  }
  ...
  try {
    ...
    await axiosInstance.post(API_PATHS.RESUMES.UPDATE(resumeId), {
      ...resumeData,
            ...
            <div className={containerStyles.previewContainer}>
              ...
              <div className="preview-container relative" ref={previewContainerRef}>
                  <div className={containerStyles.previewInner}>
                      <RenderResume key={`preview-${resumeData?.template?.theme}`} resumeData={resumeData} 
                  ...
                  <div className={containerStyles.pdfPreview}>
                      <div ref={resumeDownloadRef} className="a4-wrapper">
                          <div className="w-full h-full">
                              <RenderResume key={`pdf-${resumeData?.template?.theme}`} resumeData={resumeData}
                                  templateId={resumeData?.template?.id || ""}
                                  containerWidth={null}
                              />
                          </div>
            ...
            {/*Now thumbnail issue fix */}
              <div style={{display: 'none'}} ref={thumbnailRef}>
                  <div className={containerStyles.hiddenThumbnail}>
                      <RenderResume key={`thumb-${resumeData?.template?.theme}`} resumeData={resumeData}
