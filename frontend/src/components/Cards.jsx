...
export const ProfileInfoCard = () => {
  ...
  const handleLogout = () => {
    clearUserData();
    navigate("/login"); // or navigate("/") if you want to land home
  };

  if (!user) return null; // Safe check

  // Use the correct user field - switch to user.fullName if backend returns that
  ...
  return (
    <div className={cardStyles.profileCard}>
      ...
      <div className={cardStyles.profileInitialsContainer}>
        <span className={cardStyles.profileInitialsText}>{initials}</span>
      </div>
      {/* Profile details */}
      <div>
        <div className={cardStyles.profileName}>{displayName}</div>
        <button
  ...
  return (
    ...
    >
      {thumbnailImg ? (
        ...
        <div className="w-full h-[200px] flex items-center justify-center bg-gradient-to-br from-violet-50 to-violet-600 to-fuchsia-50">
